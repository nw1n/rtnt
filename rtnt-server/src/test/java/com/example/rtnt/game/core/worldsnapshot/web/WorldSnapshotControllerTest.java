...
class WorldSnapshotControllerTest {
    ...
    void getAllReturnsTickAndIslandPopulations() throws Exception {
        ...
        when(this.worldSnapshotStore.list()).thenReturn(List.of(
                new WorldSnapshot(0, List.of(island), List.of(new IslandStatus(island.id(), 12, Inventory.empty()))),
                new WorldSnapshot(1000, List.of(island), List.of(new IslandStatus(island.id(), 18, Inventory.empty())))
        ));
