...
class WorldSnapshotControllerTest {
    ...
    @Test
    void getAllReturnsIslandStatusInventoriesAndPrices() throws Exception {
        ...
        when(this.worldSnapshotStore.list()).thenReturn(List.of(
                ...
                new WorldSnapshot(
                        1000,
                        List.of(island),
                        List.of(new IslandStatus(island.id(), 18, Inventory.empty(), TradePriceList.defaultPrices())),
                        List.of()
                )
        ));
        ...
        this.mockMvc.perform(get("/api/world-snapshots"))
                ...
                .andExpect(jsonPath("$[0].islands[0].population").doesNotExist())
                .andExpect(jsonPath("$[0].islands[0].inventory").doesNotExist())
                .andExpect(jsonPath("$[0].islands[0].tradePrices").doesNotExist())
                .andExpect(jsonPath("$[0].islandStatuses[0].islandId").value(island.id()))
